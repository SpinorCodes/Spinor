...
int main ()
{
  ...
  float              lambda;                                                                        // Lamé 1st parameter [];
  float              mu;                                                                            // Lamé 2nd parameter [];
  float              q;                                                                             // Dissipative to direct momentum flow ratio [].
  ...
  K               = E/(3*(1 - 2*nu));                                                               // Computing bulk modulus...
  k               = K*ds*(9/(6 + 12*sqrt (2)));                                                     // Computing spring constant (3D isotropic 18-node cubic MSM)...
  lambda          = (E*nu)/((1 + nu)*(1 - 2*nu));                                                   // Computing 1st Lamé parameter...
  mu              = E/(2*(1 + nu));                                                                 // Computing 2nd Lamé parameter...
  q               = (lambda - mu)/(2*(5/3)*mu);                                                     // Computing dissipative to direct momentum flow ratio...
  dt_critical     = sqrt (dm/k);
  dt_simulation   = 0.2*dt_critical;
  ...
  std::cout << "Bulk modulus = " << K << " [Pa]" << std::endl;                                      // Printing message...
  std::cout << "Lame's 1st parameter = " << lambda << " []" << std::endl;                           // Printing message...
  std::cout << "Lame's 2nd parameter = " << mu << " []" << std::endl;                               // Printing message...
  std::cout << "Dissipative to direct momentum flow ratio = " << q << " []" << std::endl;           // Printing message...
