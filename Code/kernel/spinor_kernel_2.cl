                        ...
                        )   
{
  ...
  for (j = j_min; j < j_max; j++)
  {
    ...
    if(
        (fabs(K) > FLT_EPSILON) &&
        (fabs(D) > FLT_EPSILON) &&
        (fabs(Fspring) > FLT_EPSILON) &&
        (fabs(R) > FLT_EPSILON)
      )
    {
      Jacc += 0.5f*D*Fspring*R;                                                       
      b += 1.0f;                                                                      // Counting 1st and 2nd nearest neighbours around the central node...
    }
    else
    {
      Jacc = 0.0f;                                                                    // Resetting radiated energy from central node...
      b = 0.0f;                                                                       // Resetting 1st and 2nd nearest neighbours around the central node...
    }
  }
